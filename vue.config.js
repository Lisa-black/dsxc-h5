...
module.exports = {
  ...
  // publicPath: defaultSettings.NODE_ENV === 'development' ? '/' : '/app/', //router history模式使用 需要区分生产环境和开发环境，不然build会报错
  outputDir: 'dist',
  assetsDir: 'static',
  lintOnSave: defaultSettings.NODE_ENV === 'development',
  ...
  chainWebpack(config) {
    ...
    config
      ...
      .when(defaultSettings.NODE_ENV === 'development', config => config.devtool('cheap-source-map'))

    config.when(defaultSettings.NODE_ENV !== 'development', config => {
      config
